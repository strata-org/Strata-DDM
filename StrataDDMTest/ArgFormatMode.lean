...
op sci (@[scientific] d : Decimal) : Command => "sci " d ";";
#end
...
def progSci   := #strata program AFM; sci 1.5; #end
...
-- The `@[scientific]` argument annotation forces the raw scientific form even
-- for an in-window decimal that the default would render plain (`1.5`),
-- exercising the arg-marker path for the `scientific` branch end-to-end.
#guard Program.toString progSci = "program AFM;\nsci 15e-1;"
