...
def StrataDDL : Dialect := BuiltinM.create! "StrataDDL" #[initDialect] do
  ...
  declareOp {
    ...
    argDecls := .ofArray #[
      ...
      { ident := "val", kind := .cat <| .atom .none TypeExpr }
    ]
    category := TypeExpr,
  ...
  declareOp {
      ...
      argDecls := .ofArray #[
        ...
        { ident := "type", kind := .cat <| .atom .none BindingType, },
        { ident := "metadata", kind := .cat <| .mkOpt (.atom .none Metadata), }
      ],
      category := Binding,
  ...
  declareOp {
      name := "mkBindings",
      argDecls := .ofArray #[
        { ident := "bs", kind := .cat <| .mkCommaSepBy <| .atom .none Binding }
      ],
      category := Bindings,
  ...
  declareOp {
    ...
    argDecls := .ofArray #[
      ...
      { ident := "args",
        kind := .cat (.mkOpt (.atom .none Bindings))
      },
      { ident := "cat",
        kind := .cat (.atom .none BindingType),
        metadata := md{ .scope 0 }
      },
      { ident := "opMetadata",
        kind := .cat (.mkOpt (.atom .none Metadata)),
        metadata := md{ .scope 1 }
      },
      { ident := "synMetadata",
        kind := .cat (.mkOpt (.atom .none Metadata)),
  ...
  declareOp {
    ...
    argDecls := .ofArray #[
      { ident := "name", kind := Ident },
      { ident := "args", kind := .cat (.mkOpt (.atom .none Bindings)) }
    ],
    category := Command,
  ...
  declareMetadata { name := "declareScopedType", args := #[.mk "name" .ident, .mk "args" (.opt .ident)] }
  ...
  /- Metadata for converting type parameter bindings (.type) to type variable bindings (.tvar).
